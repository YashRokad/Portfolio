...
export function seed({ force = false } = {}) {
  ...
  return { projects: projects.length, testimonials: testimonials.length, media: dir };
}

/** Called on server boot — only writes when content files are missing. */
export function ensureSeeded() {
  if (!exists('projects') || !exists('about') || !exists('testimonials')) seed();
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  console.log(`[seed] wrote ${result.projects} projects, ${result.testimonials} testimonials, about.json`);
